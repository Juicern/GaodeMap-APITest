...
public class LoginActivity extends Activity {
    ...
    private void init() {
        //获得缓存
        sp = getSharedPreferences("gaodemap", Context.MODE_PRIVATE);

        editor = sp.edit();
        //从缓存中读入账号
        String curAccount = sp.getString("etUsername", null);
        //从缓存中读入密码
        String curPassword = sp.getString("etLoginPassword", null);
        if(curAccount != null && curAccount.length() != 0) {
            etUsername.setText(curAccount);
        }
        if(curPassword != null && curPassword.length() != 0) {
            etLoginPassword.setText(curPassword);
    ...
    private void login(){
        ...
        if(etUsername.getText().toString().trim().equals(Utils.get(LoginActivity.this,"phone"))
         && etLoginPassword.getText().toString().trim().equals(Utils.get(LoginActivity.this,"password"))){
            editor.putString("etUsername", etUsername.getText().toString().trim());
            if(rememberAccount.isChecked()) {
                editor.putString("etLoginPassword", etLoginPassword.getText().toString().trim());
            }
            else {
                //若“记住账号米密码”框未选中，则从缓存中移除密码
                editor.remove("etLoginPassword");
                editor.putString("etLoginPassword", null);
            }
            editor.commit();
